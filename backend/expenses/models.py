...
class Expense(models.Model):
    ...
    converted_amount_inr = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        if not self.converted_amount_inr:
            if self.currency == self.group.currency:
                self.converted_amount_inr = self.amount
            else:
                try:
                    ...
                    self.converted_amount_inr = self.amount * rate_obj.rate
